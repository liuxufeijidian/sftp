...
fis.match('/components/{*,**/*}.{otf,eot,svg,ttf,woff,woff2,json}', {
	release : '/assets/$0',
});

fis.match('/components/{*,**/*}.{md,map,gzip}', {
	release : false,
});

fis.match('/modules/{*,**/*}.{js,css,jpg,png,gif}', {
	isMod : true,
	id : '$0',
	useHash : true,
	release : '/assets/$0',
...
fis.media('deploy').match('*', {
	deploy : fis.plugin('ftp', {
		...
		remoteDir : '/operate/', // 远程文件目录，注意！！！设置错误将导致文件被覆盖
		connect : {
			host : '139.129.16.229',
			port : '21',
			user : 'ftp',
...
fis.set('domain', 'http://test.chejiache.net:8080/operate');
